...
python3 scripts/score_growth_stock.py --input data/nbis_metrics.json
...
python3 scripts/assemble_growth_report.py --ticker NBIS --analysis-date 2026-04-02 --metrics data/nbis_metrics.json --score data/nbis_score.json --backtest data/nbis_backtest.json --notes data/nbis_notes.json --output data/nbis_report.json
...
### 5. Scorecard-driven review

1. Extract a compact metrics JSON from filings or earnings materials.
2. Use `scripts/extract_growth_metrics.py` for a first pass if the source is plain text or markdown.
2. Score it with `scripts/score_growth_stock.py`.
3. Use the score as a consistency check, not as a substitute for judgment.
...
### 10. Earnings-season batch refresh
