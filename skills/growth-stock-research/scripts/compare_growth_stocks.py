...
def render_markdown(left: dict, right: dict) -> str:
    ...
    lines.extend(
        [
            ...
            f"- {left_name} valuation engine: {metric(left, 'valuation_summary', 'No valuation snapshot attached')}",
            f"- {right_name} valuation engine: {metric(right, 'valuation_summary', 'No valuation snapshot attached')}",
            "",
            "## Evidence Quality",
            f"- {left_name}: {metric(left, 'evidence_summary', 'No evidence-quality snapshot attached')}",
            f"- {right_name}: {metric(right, 'evidence_summary', 'No evidence-quality snapshot attached')}",
            "",
            "## Market Behavior",
            f"- {left_name} relative strength: {metric(left, 'relative_strength_summary', 'No relative-strength snapshot attached')}",
            f"- {right_name} relative strength: {metric(right, 'relative_strength_summary', 'No relative-strength snapshot attached')}",
            f"- {left_name} event study: {metric(left, 'event_study_summary', 'No event-study snapshot attached')}",
            f"- {right_name} event study: {metric(right, 'event_study_summary', 'No event-study snapshot attached')}",
            "",
            "## Backtest Snapshot",
